...
export function PlayerTank({ playerId }: Props) {
  ...
  useFrame((_, delta) => {
    ...
    if (playerId === "player1") {
      if (keys.has("ArrowUp") || keys.has("KeyW")) direction = "DOWN";
      else if (keys.has("ArrowDown") || keys.has("KeyS")) direction = "UP";
    ...
    if (direction) {
      ...
      if (result.canMove) {
        usePlayerStore
          .getState()
          .setPosition(playerId, result.newX, result.newY);
      }
    ...
    if (keys.has(shootKey)) {
      ...
      if (now - lastShootRef.current > 800) {
  ...
  const player = usePlayerStore(
    (s) => s[playerId === "player2" ? "player2" : "player1"],
  );
